...
import { Firestore, addDoc, collection, doc, getDocs, onSnapshot, query, updateDoc, where } from 'firebase/firestore';
...
export class HuishoudboekjeService {
  ...
  readHuishoudboekjes(user: User | null, isArchived: boolean): Observable<Huishoudboekje[]> {
    if(user === null) {
    ...
    return new Observable((subscriber: Subscriber<any[]>) => {
      onSnapshot(collection(this.firestore, 'huishoudboekjes'), (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          if ((data.owner === user.email || (data.participants !== undefined && data.participants.includes(user.email))) && data.archive === isArchived) {
