...
export class BarChartComponent {
  ...
  public initColomChartData(huishoudBoekjeID: string): void {
    ...
    const categorieData$: Observable<Categorie[]> = this._categorieService.readAll();
    ...
    categorieData$.subscribe((categorieen) => {
      ...
      const data$: Observable<Transactie[]> = initialData$.pipe(
        mergeMap(() =>
          this.transactieService.readTransactiesOfHuishoudboekje(huishoudBoekjeID)
        )
      );
      ...
      data$.subscribe((transacties: Transactie[]) => {
        ...
        const categoryDataMap = new Map<string, { uitgaven: number; inkomsten: number }>();
        ...
        transacties.forEach((transaction: Transactie) => {
          ...
          if (category) {
            const categoryName = categorieNaamMap.get(category) || category;
            const existingData = categoryDataMap.get(categoryName) || { uitgaven: 0, inkomsten: 0 };
        ...
        const uitgavenData = Array.from(categoryDataMap.values(), (data) => data.uitgaven);
        const inkomstenData = Array.from(categoryDataMap.values(), (data) => data.inkomsten);
